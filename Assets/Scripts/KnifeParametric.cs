...
public class KnifeParametric : MonoBehaviour
{
    ...
    public List<Symbol> RewriteStringCrescent(List<Symbol> inputString)
    {
        List<Symbol> newString = new List<Symbol>();
        foreach (var item in inputString)
        {
            switch (item.s)
            {
                ...
                case "Blade":
                    newString.Add(new Symbol("crescent", new object[] { crescentL, crescentW, crescentD, crescentT, edgeRatio, circleSubdivision }));
                    break;
